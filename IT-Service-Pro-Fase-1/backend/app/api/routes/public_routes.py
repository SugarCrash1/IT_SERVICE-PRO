...
def chat_publico(datos: ChatRequest, db: Session = Depends(get_db)):
    """Envía un mensaje al asistente. Guarda el mensaje y la respuesta en chat_mensajes,
    indexados por sesion_id, para que la conversación sobreviva a un F5."""
    ...
    proveedor = PROVEEDORES.get(provider_name, _llamar_local)

    if not _es_tema_del_salon(datos.mensaje, bool(datos.imagen_base64)):
        texto = "Puedo ayudarte únicamente con servicios, productos, citas y cuidados de belleza del salón. ¿Qué deseas mejorar?"
        _guardar_mensaje(db, datos.sesion_id, "user", datos.mensaje.strip())
        _guardar_mensaje(db, datos.sesion_id, "assistant", texto)
        return {"data": {"respuesta": texto, "recomendaciones": [], "origen": "filtro_negocio"}}

    historial = _historial_sesion(db, datos.sesion_id)
    system = _system_prompt(db)

    _guardar_mensaje(db, datos.sesion_id, "user", datos.mensaje.strip() or "[imagen enviada]")

    inicio = time.perf_counter()
    origen_respuesta = "local"

    # Las consultas frecuentes del salón se resuelven localmente en milisegundos.
    # La IA externa queda para preguntas más abiertas, evitando esperas innecesarias.
    if provider_name == "local" or _debe_responder_local_rapido(datos):
        texto = _llamar_local(system, historial, datos, db)
    else:
        try:
            texto = proveedor(system, historial, datos)
            origen_respuesta = provider_name
        except Exception as exc:
            logger.warning("Fallback local del chat. proveedor=%s error=%s", provider_name, exc)
            texto = _llamar_local(system, historial, datos, db)

    logger.info(
        "Chat respondido. proveedor_configurado=%s origen=%s duracion_ms=%s sesion=%s",
        provider_name, origen_respuesta, round((time.perf_counter() - inicio) * 1000), datos.sesion_id
    )
    texto = texto or "No pude generar una respuesta, intenta reformular tu pregunta."
    _guardar_mensaje(db, datos.sesion_id, "assistant", texto)

    recomendaciones = _seleccionar_recomendaciones(db, f"{datos.mensaje} {texto}")
    return {"data": {"respuesta": texto, "recomendaciones": recomendaciones, "origen": origen_respuesta}}
