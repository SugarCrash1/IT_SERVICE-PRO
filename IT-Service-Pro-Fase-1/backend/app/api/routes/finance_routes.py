...
def mis_comisiones(fecha_desde:date|None=Query(None),fecha_hasta:date|None=Query(None),
 user:Usuario=Depends(require_roles(RolesSistema.TECNICO)),db:Session=Depends(get_db)):
...
def cobros_pendientes(user:Usuario=Depends(require_roles(RolesSistema.ADMINISTRADOR,RolesSistema.COORDINADOR)),db:Session=Depends(get_db)):
...
def historial_caja(pagina:int=Query(1,ge=1),tamano:int=Query(20,ge=1,le=100),
 user:Usuario=Depends(require_roles(RolesSistema.ADMINISTRADOR,RolesSistema.COORDINADOR)),db:Session=Depends(get_db)):
