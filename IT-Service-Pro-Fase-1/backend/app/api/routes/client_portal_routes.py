...
# ---------------------------------------------------------------------------
# Portal empresarial de tickets: para contactos de empresas cliente (IT
# Service Pro). Usa Usuario.contacto_empresa_id en vez de Usuario.cliente_id.
# ---------------------------------------------------------------------------
from app.core.exceptions import ForbiddenException, NotFoundException
from app.models.itsp_foundation_model import ContactoEmpresa, EmpresaCliente, ServicioTI
from app.schemas.ticket_schema import (
    TicketAdjuntoCrearRequest, TicketComentarioCrearRequest, TicketPortalCrearRequest, TicketResponse, TicketSatisfaccionRequest,
)
from app.services.ticket_service import TicketService
from app.api.routes.ticket_routes import _response as _ticket_response


def _contacto(user:Usuario, db:Session) -> ContactoEmpresa:
    if not user.contacto_empresa_id:
        raise UnauthorizedException('La cuenta no está vinculada a un contacto de empresa')
    contacto = db.query(ContactoEmpresa).filter(ContactoEmpresa.id==user.contacto_empresa_id).first()
    if not contacto:
        raise UnauthorizedException('El contacto asociado a la cuenta ya no existe')
    return contacto


@router.get('/company')
def mi_empresa(user:Usuario=Depends(get_current_user),db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    empresa=db.query(EmpresaCliente).filter(EmpresaCliente.id==contacto.empresa_id).first()
    if not empresa: raise NotFoundException('Empresa no encontrada')
    contactos=db.query(ContactoEmpresa).filter(ContactoEmpresa.empresa_id==empresa.id).order_by(ContactoEmpresa.apellidos).all()
    return RespuestaExito(data={
        'empresa':{'id':str(empresa.id),'razon_social':empresa.razon_social,'nombre_comercial':empresa.nombre_comercial,
            'ruc':empresa.ruc,'sector':empresa.sector,'nivel_sla':empresa.nivel_sla,'ciudad':empresa.ciudad,
            'telefono':empresa.telefono,'correo':empresa.correo,'contrato_fin':empresa.contrato_fin,'logo_url':empresa.logo_url},
        'mi_contacto':{'id':str(contacto.id),'nombres':contacto.nombres,'apellidos':contacto.apellidos,
            'cargo':contacto.cargo,'correo':contacto.correo,'es_contacto_principal':contacto.es_contacto_principal},
        'contactos':[{'id':str(c.id),'nombres':c.nombres,'apellidos':c.apellidos,'cargo':c.cargo,
            'correo':c.correo,'es_contacto_principal':c.es_contacto_principal} for c in contactos],
    })


@router.get('/services-catalog')
def catalogo_servicios_portal(db:Session=Depends(get_db)):
    items=db.query(ServicioTI).filter(ServicioTI.estado=='ACTIVO').order_by(ServicioTI.categoria,ServicioTI.nombre).all()
    return RespuestaExito(data=[{'id':str(s.id),'codigo':s.codigo,'nombre':s.nombre,'categoria':s.categoria,
        'descripcion':s.descripcion,'tiempo_respuesta_horas':s.tiempo_respuesta_horas} for s in items])


@router.get('/tickets', response_model=RespuestaExito[dict])
def mis_tickets(estado:str|None=Query(None), pagina:int=Query(1,ge=1), tamano_pagina:int=Query(20,ge=1,le=100),
    user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    items,total=TicketService(db).listar(empresa_id=contacto.empresa_id, estado=estado, pagina=pagina,
        tamano_pagina=tamano_pagina, orden_por='created_at', orden_direccion='desc')
    from app.api.routes.ticket_routes import _list_item
    data=[_list_item(x).model_dump() for x in items]
    total_paginas=(total+tamano_pagina-1)//tamano_pagina if tamano_pagina else 0
    return RespuestaExito(data={'items':data,'total':total,'pagina':pagina,'tamano_pagina':tamano_pagina,'total_paginas':total_paginas})


@router.get('/tickets/summary')
def resumen_tickets_portal(user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    items,total=TicketService(db).listar(empresa_id=contacto.empresa_id, pagina=1, tamano_pagina=1000)
    abiertos=sum(1 for t in items if t.estado in ('ABIERTO','EN_PROGRESO','EN_ESPERA_CLIENTE','EN_ESPERA_TERCERO'))
    resueltos=sum(1 for t in items if t.estado in ('RESUELTO','CERRADO'))
    return RespuestaExito(data={'total':total,'abiertos':abiertos,'resueltos':resueltos})


@router.post('/tickets', response_model=RespuestaExito[TicketResponse], status_code=201)
def crear_ticket_portal(datos:TicketPortalCrearRequest, user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    ticket=TicketService(db).crear_desde_portal(datos, contacto)
    return RespuestaExito(data=_ticket_response(ticket, incluir_internos=False), message=f'Ticket {ticket.codigo} creado correctamente')


@router.get('/tickets/{ticket_id}', response_model=RespuestaExito[TicketResponse])
def obtener_ticket_portal(ticket_id:uuid.UUID, user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    svc=TicketService(db)
    ticket=svc.obtener(ticket_id)
    if str(ticket.empresa_id)!=str(contacto.empresa_id):
        raise ForbiddenException('No puede consultar tickets de otra empresa')
    return RespuestaExito(data=_ticket_response(ticket, incluir_internos=False))


@router.post('/tickets/{ticket_id}/comments', response_model=RespuestaExito[TicketResponse], status_code=201)
def comentar_ticket_portal(ticket_id:uuid.UUID, datos:TicketComentarioCrearRequest,
    user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    svc=TicketService(db)
    ticket=svc.obtener(ticket_id)
    if str(ticket.empresa_id)!=str(contacto.empresa_id):
        raise ForbiddenException('No puede comentar tickets de otra empresa')
    ticket=svc.comentar(ticket_id, datos, contacto=contacto)
    return RespuestaExito(data=_ticket_response(ticket, incluir_internos=False), message='Comentario agregado')


@router.post('/tickets/{ticket_id}/attachments', response_model=RespuestaExito[TicketResponse], status_code=201)
def adjuntar_ticket_portal(ticket_id:uuid.UUID, datos:TicketAdjuntoCrearRequest,
    user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    svc=TicketService(db)
    ticket=svc.obtener(ticket_id)
    if str(ticket.empresa_id)!=str(contacto.empresa_id):
        raise ForbiddenException('No puede adjuntar archivos a tickets de otra empresa')
    ticket=svc.adjuntar(ticket_id, datos)
    return RespuestaExito(data=_ticket_response(ticket, incluir_internos=False), message='Archivo adjuntado')


@router.post('/tickets/{ticket_id}/satisfaction', response_model=RespuestaExito[TicketResponse])
def calificar_ticket_portal(ticket_id:uuid.UUID, datos:TicketSatisfaccionRequest,
    user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    contacto=_contacto(user,db)
    ticket=TicketService(db).calificar(ticket_id, datos, contacto)
    return RespuestaExito(data=_ticket_response(ticket, incluir_internos=False), message='Gracias por tu calificación')


@router.get('/assets')
def mis_activos(user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    from app.models.asset_model import Activo
    from app.api.routes.asset_routes import _response as _activo_response
    contacto=_contacto(user,db)
    items=db.query(Activo).options(joinedload(Activo.empresa),joinedload(Activo.responsable)).filter(
        Activo.empresa_id==contacto.empresa_id).order_by(Activo.created_at.desc()).all()
    return RespuestaExito(data=[_activo_response(a) for a in items])


@router.get('/projects')
def mis_proyectos(user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    from app.models.project_model import Proyecto, ProyectoTarea
    from app.api.routes.project_routes import _response as _proyecto_response
    contacto=_contacto(user,db)
    items=db.query(Proyecto).options(
        joinedload(Proyecto.empresa), joinedload(Proyecto.responsable),
        joinedload(Proyecto.tareas).joinedload(ProyectoTarea.responsable),
    ).filter(Proyecto.empresa_id==contacto.empresa_id).order_by(Proyecto.created_at.desc()).all()
    return RespuestaExito(data=[_proyecto_response(p) for p in items])


@router.get('/documents')
def mis_documentos(user:Usuario=Depends(get_current_user), db:Session=Depends(get_db)):
    from app.models.document_model import Documento
    from app.api.routes.document_routes import _response as _documento_response
    contacto=_contacto(user,db)
    items=db.query(Documento).options(joinedload(Documento.empresa),joinedload(Documento.subido_por)).filter(
        Documento.empresa_id==contacto.empresa_id, Documento.visible_portal.is_(True)).order_by(Documento.created_at.desc()).all()
    return RespuestaExito(data=[_documento_response(d) for d in items])
