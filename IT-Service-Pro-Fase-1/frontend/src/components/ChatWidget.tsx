...
type Msg={role:'user'|'assistant';content:string;imagenPreview?:string;recomendaciones?:Recommendation[]};
const SALUDO:Msg={role:'assistant',content:'¡Hola! Soy el asistente de Elegance. Puedo ayudarte únicamente con servicios, productos, citas y cuidados de belleza del salón.'};
...
export function ChatWidget(){
 ...
 const send=async()=>{
  const text=input.trim();if((!text&&!image)||sending)return;
  ...
  try{
   ...
   setMessages(v=>[...v,{role:'assistant',content:data.data.respuesta,recomendaciones:data.data.recomendaciones||[]}]);
  }catch(e:any){
