...
export const fileUrl=(path?:string|null)=>{if(!path)return '';return path.startsWith('http')?path:`${serverOrigin}${path}`};
...
export const errorMessage=(e:any)=>e?.response?.data?.message||e?.response?.data?.detail||e?.response?.data?.mensaje||e?.message||'Ocurrió un error';
